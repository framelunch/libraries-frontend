'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

exports.default = createState;

var _Cp = require('./Cp');

var _Cp2 = _interopRequireDefault(_Cp);

var _loop2 = require('../utils/loop');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function _set(prefix, items, pool, cps, arg) {
  for (var i = 0, l = items.length, name; i < l; i += 1) {
    name = items[i];
    pool.push({ prefix: prefix, name: name, arg: arg });
    name = prefix + name;
    if (!cps[name]) {
      cps[name] = (0, _Cp2.default)();
    }
  }
}

var Data = function () {
  function Data(value) {
    _classCallCheck(this, Data);

    this.data = this.make(value);
    this.str = '/' + this.data;
    this.array = this.data === '' ? [''] : this.data.split('/');
  }

  _createClass(Data, [{
    key: 'make',
    value: function make(value) {
      if (value.charAt(0) === '/') {
        value = value.substr(1);
      }
      if (value.charAt(value.length - 1) === '/') {
        value = value.substr(0, value.length - 1);
      }
      return value;
    }
  }, {
    key: 'compare',
    value: function compare(value) {
      var arr = [],
          l = this.array.length;
      var i = void 0,
          a = void 0,
          b = void 0;

      for (i = 0; i < l; i += 1) {
        a = this.array.slice(0, i).join('/');
        b = value.array.slice(0, i).join('/');

        if (a + this.array[i] !== b + value.array[i]) {
          arr.push(this.array[i]);
        }
      }
      return arr;
    }
  }, {
    key: 'eq',
    value: function eq(value) {
      return this.data === this.make(value);
    }
  }]);

  return Data;
}();

var State = function () {
  function State(def) {
    var _this = this;

    _classCallCheck(this, State);

    this.pool = [];
    this.active = false;
    this.w = 0;
    this.current = new Data(def || '');
    this.cps = { 'state:start': (0, _Cp2.default)(), 'state:update': (0, _Cp2.default)(), 'state:end': (0, _Cp2.default)() };
    this.loop = function () {
      return _this._loop();
    };
    this.isHistory = false;
    this.historys = [];
    this.ite = 0;
  }

  _createClass(State, [{
    key: '_loop',
    value: function _loop() {
      if (this.w) {
        return;
      }

      if (this.pool.length === 0) {
        this.active = false;
        this.cps['state:end'].update();
        (0, _loop2.removeFromLoop)(this.loop);
        return;
      }

      var o = this.pool[0];
      this.cps[o.prefix + o.name].update(o.arg);
      this.pool.shift();
    }
  }, {
    key: '_change',
    value: function _change(v, arg) {
      if (this.current.eq(v)) {
        return false;
      }
      var data = new Data(v);
      _set('remove:', this.current.compare(data).reverse(), this.pool, this.cps, arg);
      _set('add:', data.compare(this.current), this.pool, this.cps, arg);

      this.current = data;

      if (!this.active) {
        this.active = true;
        this.cps['state:start'].update();
        (0, _loop2.addToLoop)(this.loop);
      }

      this.cps['state:update'].update([this.current]);
      return true;
    }
  }, {
    key: 'change',
    value: function change(v, arg) {
      if (this._change(v, arg)) {
        if (this.isHistory) {
          if (this.historys.length === 0) {
            this.historys.push(this.current.data);
          } else {
            this.historys.splice(this.ite + 1);
            this.historys.push(this.current.data);
            this.ite += 1;
          }
        }
        return true;
      }
      return false;
    }
  }, {
    key: 'prev',
    value: function prev() {
      if (!this.isHistory) return;
      if (this.ite < 1) return;
      this.ite -= 1;
      this._change(this.historys[this.ite]);
    }
  }, {
    key: 'next',
    value: function next() {
      if (!this.isHistory) return;

      var l = this.historys.length;
      if (l === 0 || this.ite === l - 1) return;
      this.ite += 1;
      this._change(this.historys[this.ite]);
    }
  }, {
    key: 'wait',
    value: function wait() {
      this.w += 1;
    }
  }, {
    key: 'notify',
    value: function notify() {
      if (this.w > 0) this.w -= 1;
    }
  }, {
    key: 'listen',
    value: function listen(key, func) {
      if (!this.cps[key]) {
        this.cps[key] = (0, _Cp2.default)();
      }
      var c = this.cps[key];
      c.add(func);
      return func;
    }
  }, {
    key: 'clear',
    value: function clear(key, func) {
      var c = this.cps[key];
      if (!c) {
        return;
      }
      c.remove(func);
    }
  }]);

  return State;
}();

function createState(def) {
  return new State(def);
}